...
AOI 0.3.0a2 final publish tooling adds the guarded real distribution path for the agent-native package. Final preflight checks version and marker sync, agent REST/MCP surfaces, package-data inclusion, no-secrets status, claim guard, tech protection, and public/private alignment. PyPI upload remains an explicit local action requiring `AOI_REAL_PYPI_UPLOAD_CONFIRM=YES` and interactive `twine` token entry. MCP Registry publish remains an explicit local action after real PyPI install verification and `AOI_MCP_REGISTRY_SUBMIT_CONFIRM=YES`. The tooling does not store tokens, commit `.pypirc`, commit dist files by default, commit `mcp-publisher`, overwrite `0.3.0a1`, certify security, prove correctness, claim legal/privacy/license clearance, guarantee quality, claim product readiness, or authorize actions.

AOI-AGENT-DISCOVERY-3 verifies the public `0.3.0a2` distribution path and creates an offline ordinary-agent prompt evaluation pack. It checks PyPI public install, MCP Registry public discovery, deterministic naive-agent failure fixtures, AOI-guided discover/preflight results, a manual cross-model evaluation sheet, and a gate result. The evaluation is synthetic and offline; it is meant to show whether AOI improves source traces, missing-field handling, route decisions, next actions, and claim boundaries compared with naive recommendations, not to prove readiness, certification, quality, or action authority.

AOI-AGENT-DISCOVERY-4 ingests manual, redacted external model feedback from Gemini, GPT-5.5 Thinking, and Claude Opus 4.8 High as local evaluation packets. It turns that feedback into a competitive synthesis, Capability Decision Packet draft, granular route-semantics roadmap, HOLD-to-Replan loop, agent/operator dual positioning note, freshness/rug-pull/negative-cache notes, and a residual test reconciliation artifact. The package classifies the prior generated registry/datascope full-suite failures as HOLD residual state and does not claim global full-suite green until a full `python -m pytest` actually passes.

AOI-AGENT-DISCOVERY-5 turns that roadmap into executable packet and route machinery. It adds the Capability Decision Packet as a canonical optional discover/preflight response object, granular route semantics and reason codes, HOLD-to-Replan, final argument preflight, freshness/rug-pull/negative-cache fields, and advisory/enforced/proxy middleware contracts. It also narrowly repairs the registry payload-state recursion residual in `real_payload_guard.py` without staging generated registry payload leftovers. D5 does not upload a new package, publish a registry version, certify security, prove readiness, clear legal/privacy/license status, or authorize actions.
