...
Package 8Q-A installs/checks local packaging tools, builds local wheel/sdist artifacts, runs `twine check`, and refreshes PyPI readiness. The resumed vNext build candidate uses `0.3.0a1`. It still does not upload to TestPyPI/PyPI or submit to MCP Registry.
