...
REDACTION_REPORT_PATH = Path("pilot_dashboard") / "ROE21_DASHBOARD_REFRESH_REDACTION_REPORT.json"
...
def build_redaction_report(findings: list[dict[str, Any]], artifact_count: int) -> dict[str, Any]:
    ...
    block_count = len([item for item in findings if item["finding_type"] in block_types])
    hold_count = len(findings) - block_count
    return {
        "schema": "ResidualOps_DashboardRefreshRedactionReport/v0.1",
...
def scan_dashboard_refresh_artifacts(paths: list[Path], write_result: bool = True) -> dict[str, Any]:
    findings: list[dict[str, Any]] = []
    for path in paths:
        full = _repo_root() / path
        if full.exists() and full.is_file():
            findings.extend(scan_text(full.read_text(encoding="utf-8", errors="ignore"), str(path).replace("\\", "/")))
