...
CLAIM_AUDIT_PATH = SHARE_DIR / "RESIDUALOPS_EXTERNAL_SAFE_CLAIM_AUDIT.json"
...
def scan_share_claim_text(text: str, label: str) -> list[dict[str, Any]]:
...
def run_share_claim_audit(paths: list[Path] | None = None, write_result: bool = True) -> dict[str, Any]:
    root = _repo_root()
    if paths is None:
    ...
    for path in paths:
        ...
        if full.exists() and full.is_file():
            findings.extend(scan_share_claim_text(full.read_text(encoding="utf-8", errors="ignore"), str(path).replace("\\", "/")))
    result = {
        "schema": "ResidualOps_ExternalSafeSharePackClaimAudit/v0.1",
