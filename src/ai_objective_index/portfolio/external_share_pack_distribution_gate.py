...
from .external_share_pack_manifest import MANIFEST_PATH, SHARE_DIR, timestamp
from .external_share_pack_redaction import REDACTION_REPORT_PATH


DISTRIBUTION_BOUNDARY_PATH = SHARE_DIR / "RESIDUALOPS_EXTERNAL_SAFE_DISTRIBUTION_BOUNDARY.json"
...
def _html_has_network_dependency() -> bool:
    html_path = _repo_root() / SHARE_DIR / "RESIDUALOPS_EXTERNAL_SAFE_DEMO.html"
    if not html_path.exists():
        return False
    lowered = html_path.read_text(encoding="utf-8", errors="ignore").lower()
    return any(marker in lowered for marker in ["<script", "<link", "<form", "http://", "https://"])


def run_distribution_gate(requested_distribution_mode: str = "local_static_demo_zip", write_result: bool = True) -> dict[str, Any]:
    ...
    if redaction.get("decision") == "BLOCK_SENSITIVE_CONTENT":
    ...
    elif _html_has_network_dependency():
        ...
        decision = "HOLD_REVIEW_REQUIRED"
    result = {
        "schema": "ResidualOps_ExternalSafeDistributionBoundary/v0.1",
        ...
        "network_dependency": _html_has_network_dependency(),
    }
    if write_result:
        destination = _repo_root() / DISTRIBUTION_BOUNDARY_PATH
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the ROE-17 external share distribution gate.")
...
def main() -> None:
    ...
    result = run_distribution_gate(args.mode, write_result=not args.no_write)
    print(f"external_share_pack_distribution_gate: {result['decision']} mode={result['requested_distribution_mode']}")
