...
from .external_share_pack_manifest import SHARE_DIR, timestamp


REDACTION_REPORT_PATH = SHARE_DIR / "RESIDUALOPS_EXTERNAL_SAFE_REDACTION_REPORT.json"
...
def scan_share_payload(payload: Any, label: str = "share_payload") -> dict[str, Any]:
    ...
    return _result(findings, artifact_count=1)


def _result(findings: list[dict[str, Any]], artifact_count: int) -> dict[str, Any]:
    block_count = len([finding for finding in findings if finding["finding_type"] in {"token_or_secret", "env_content", "private_kernel_value", "unsafe_file"}])
    hold_count = len(findings) - block_count
    return {
        "schema": "ResidualOps_ExternalSafeSharePackRedactionReport/v0.1",
...
def scan_share_pack_artifacts(paths: list[Path] | None = None, write_result: bool = True) -> dict[str, Any]:
    root = _repo_root()
    if paths is None:
        paths = sorted(path.relative_to(root) for path in (root / SHARE_DIR).glob("*") if path.is_file())
    findings: list[dict[str, Any]] = []
    for path in paths:
    ...
    report = _result(findings, artifact_count=len(paths))
    if write_result:
