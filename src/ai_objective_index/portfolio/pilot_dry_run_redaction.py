...
REDACTION_REPORT_PATH = DRY_RUN_DIR / "ROE13_PILOT_DRY_RUN_REDACTION_REPORT.json"
...
def scan_dry_run_payload(payload: Any, label: str = "pilot_dry_run_payload") -> dict[str, Any]:
    ...
    block_types = {"token_or_secret", "env_content", "private_kernel_value"}
    ...
    return {
        "schema": "ResidualOps_PilotDryRunRedactionReport/v0.1",
...
def scan_dry_run_artifacts(paths: list[Path], write_result: bool = True) -> dict[str, Any]:
    findings: list[dict[str, Any]] = []
    for path in paths:
