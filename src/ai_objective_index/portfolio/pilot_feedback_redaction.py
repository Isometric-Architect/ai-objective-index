...
REDACTION_REPORT_PATH = FEEDBACK_DIR / "PILOT_FEEDBACK_REDACTION_REPORT.json"
...
def _scan_text(text: str, label: str) -> list[dict[str, Any]]:
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if RAW_DATASET_ROW_PATTERN.search(line):
            findings.append({"label": label, "line": line_number, "finding_type": "raw_private_dataset_row"})
            continue
...
def scan_feedback_payload(payload: Any, label: str = "pilot_feedback_payload") -> dict[str, Any]:
...
def _build_report(findings: list[dict[str, Any]], artifact_count: int, label: str | None = None) -> dict[str, Any]:
    block_types = {"token_or_secret", "env_content", "private_kernel_value", "raw_private_dataset_row"}
...
def scan_feedback_artifacts(paths: list[Path], write_result: bool = True) -> dict[str, Any]:
    findings: list[dict[str, Any]] = []
    for path in paths:
