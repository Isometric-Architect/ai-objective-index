...
REDACTION_REPORT_PATH = PORTFOLIO_DIR / "RESIDUALOPS_PORTFOLIO_REDACTION_REPORT.json"
...
RAW_DATASET_ROW_PATTERN = re.compile(r"^[A-Za-z0-9_\- ]{1,80},[A-Za-z0-9_\- .@]{1,120},[A-Za-z0-9_\- .@]{1,120}(?:,|$)")
...
def _scan_text(text: str, label: str) -> list[dict[str, Any]]:
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if any(pattern.search(line) for pattern in PII_PATTERNS):
            ...
            continue
        if RAW_DATASET_ROW_PATTERN.search(line) and "raw_dataset" in label.lower():
            findings.append({"label": label, "line": line_number, "finding_type": "raw_dataset_row"})
    return findings


def scan_portfolio_payload(payload: Any, label: str = "portfolio_payload") -> dict[str, Any]:
    ...
    return {
        "schema": "ResidualOps_PortfolioRedactionReport/v0.1",
...
def scan_portfolio_artifacts(paths: list[Path]) -> dict[str, Any]:
    findings: list[dict[str, Any]] = []
    for path in paths:
